...
function buildResolutionItem({ item = {}, index = 0, files = [] }) {
  ...
  const selectedReferenceAssets = selectDistinctReferenceAssets(classified.assets
...
function selectDistinctReferenceAssets(assets = []) {
  const seenRoles = new Set();
  const selected = [];
  for (const asset of assets) {
    const role = inferReferenceViewRole(asset.name || asset.path || "");
...
function inferReferenceViewRole(value = "") {
  const text = String(value || "").toLowerCase();
  if (/(?:^|[\s._-])front(?:[\s._-]|$)|หน้า/.test(text)) return "front";
  if (/(?:^|[\s._-])back(?:[\s._-]|$)|หลัง/.test(text)) return "back";
  if (/(?:^|[\s._-])side(?:[\s._-]|$)|ข้าง/.test(text)) return "side";
  if (/(?:^|[\s._-])(inside|inner|lining|interior)(?:[\s._-]|$)|ซับใน|ด้านใน/.test(text)) return "inside";
  const extraMatch = text.match(/(?:^|[\s._-])(extra\d+)(?:[\s._-]|$)/);
