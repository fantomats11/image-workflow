...
export function buildStudioMasterPromptV3() {
  return [
...
export function buildSupportPromptV3(item = {}, shotKey, index, total) {
  const brief = buildPdpSupportStudioBriefV3(item, shotKey);
  const hasApprovedHeroAnchor = Boolean(item.approved_hero_anchor || item.approvedHeroAnchor);
  const hasApprovedStudioMasterAnchor = Boolean(item.approved_studio_master_anchor || item.approvedStudioMasterAnchor);
  return [
    hasApprovedStudioMasterAnchor
      ? "อ้างอิงภาพหลักที่อนุมัติแล้ว Studio Master ที่อนุมัติแล้ว และภาพสินค้าจริงจาก catalog/Drive"
      : hasApprovedHeroAnchor
        ? "อ้างอิงภาพหลักที่อนุมัติแล้วและภาพสินค้าจริงจาก catalog/Drive"
        : "อ้างอิงภาพสินค้าจริงจาก catalog/Drive",
    ...buildSupportReferenceRoleLines({
      hasApprovedHeroAnchor,
      hasApprovedStudioMasterAnchor,
      shotKey
    }),
    ...
    "",
    buildPdpSupportCreateLine(brief),
    buildLeanThaiSupportTruthLine(shotKey),
    "รูปจริงจาก catalog/Drive ใช้เป็น source of truth ของสินค้าเท่านั้น ไม่ใช้เป็น output โดยตรง",
    buildPdpSupportPresentationLine(shotKey, brief)
  ].filter(Boolean).join("\n");
}
...
function buildPdpSupportStudioBriefV3(item = {}, shotKey = "") {
  const { group } = resolveProductUseCaseV3(item);
  return {
    angle: resolvePdpSupportAngle({ group, shotKey }),
    productCategory: resolvePdpProductCategory(item, group),
    keyDetail: resolvePdpKeyDetail({ group, shotKey, item })
  };
}

function resolvePdpSupportAngle({ group = "", shotKey = "" } = {}) {
  if (["lining_warmth", "material_or_lining_closeup", "open_interior"].includes(shotKey)) {
    return "ซูมรายละเอียดด้านใน (Interior View)";
  }
  if (["texture_closeup", "texture_construction_closeup"].includes(shotKey)) {
    return group === "gloves" || group === "scarf_accessory" || group === "socks" || group === "hat"
      ? "เจาะลึกรายละเอียดอุปกรณ์ (Close-up Detailed View)"
      : "เจาะลึกรายละเอียดวัสดุและงานประกอบ (Close-up Detailed View)";
  }
  if (["back_fit_on_model", "back_hood_closure"].includes(shotKey)) {
    return group === "pants"
      ? "ด้านหลังเต็มตัวแบบวางราบ (Full Flat Lay Back View)"
      : "ด้านหลังแบบสตูดิโอ (Studio Back View)";
  }
  if (["side_fit_on_model", "side_thickness_length", "side_profile", "side_view"].includes(shotKey)) {
    return "ด้านข้างแบบสตูดิโอ (Studio Side View)";
  }
  if (shotKey === "sole_view") return "เจาะรายละเอียดพื้นรองเท้า (Outsole Detail View)";
  if (shotKey === "palm_side") return "ด้านฝ่ามือแบบเจาะรายละเอียด (Palm Detail View)";
  if (["waistband_pocket", "hem_detail"].includes(shotKey)) return "เจาะรายละเอียดโครงสร้างกางเกง (Construction Detail View)";
  if (["front_pair", "pair_front", "front_view", "front_fit_shape"].includes(shotKey)) return "ด้านหน้าแบบสตูดิโอ (Studio Front View)";
  return "มุมสินค้าเพิ่มเติมสำหรับหน้า PDP (PDP Support View)";
}

function resolvePdpProductCategory(item = {}, group = "") {
  const productName = normalizeSpaces(item.product_name || item.productName || "");
  const subcategory = normalizeSpaces(item.subcategory || "");
  const category = normalizeSpaces(item.category || "");
  const text = `${productName} ${subcategory} ${category}`.toLowerCase();
  ...
  return subcategory || category || productName || "สินค้า";
}

function resolvePdpKeyDetail({ group = "", shotKey = "", item = {} } = {}) {
  if (["lining_warmth", "material_or_lining_closeup", "open_interior"].includes(shotKey)) {
    return "ตัวอักษรบนป้ายแคร์ลาเบล (Care Label) และความสะอาดของเนื้อผ้าซับในด้านใน";
  }
  if (group === "pants") {
    if (shotKey === "back_hood_closure" || shotKey === "back_fit_on_model") return "ทรงกางเกง ตะเข็บเย็บ และกระเป๋าหลังทั้งหมด";
    if (shotKey === "waistband_pocket") return "เอว กระดุม drawcord กระเป๋า ตะเข็บ และโครงสร้างช่วงบน";
    if (shotKey === "hem_detail") return "ชายขา ขอบปลาย ตะเข็บ ความหนาผ้า และรูปทรงปลายขา";
    return "ทรงกางเกง ตะเข็บ กระเป๋า ผิวผ้า และรายละเอียดการตัดเย็บ";
  }
  if (group === "gloves") {
    if (shotKey === "palm_side" || shotKey === "texture_closeup" || shotKey === "texture_construction_closeup") {
      return "พื้นผิวกันลื่นตรงฝ่ามือ (Grip Texture) และสายรัดข้อมือ";
    }
    return "รูปทรงนิ้ว ขอบข้อมือ ตะเข็บ วัสดุ และความหนาของถุงมือ";
  }
  if (group === "footwear") {
    if (shotKey === "sole_view") return "ลายพื้นรองเท้า ความหนา ขอบพื้น และพื้นผิวกันลื่น";
  ...
  if (["back_fit_on_model", "back_hood_closure"].includes(shotKey)) return "ดีไซน์ด้านหลัง ฮู้ด ทรงไหล่ ความยาว ตะเข็บ และโลโก้หรือแพตช์จริงถ้ามี";
  if (["side_fit_on_model", "side_thickness_length", "side_view"].includes(shotKey)) return "ทรงด้านข้าง ความหนา ความยาว ปลายแขน ซิป ตะเข็บ และสัดส่วนสินค้า";
  if (["texture_closeup", "texture_construction_closeup"].includes(shotKey)) return "พื้นผิววัสดุ งานเย็บ ซิป ป้ายจริง ขอบผ้า และรายละเอียดที่ช่วยตัดสินใจ";
  return describeLeanThaiSupportShotV3(shotKey, item);
}

function buildPdpSupportCreateLine({ angle = "", productCategory = "", keyDetail = "" } = {}) {
  return [
    `สร้างภาพสนับสนุนหน้า PDP ของ${productCategory} ในมุม${angle}`,
    "ให้ดูเรียล โปร่งใส และน่าเชื่อถือ สื่อถึงสินค้าของแท้ในสภาพที่สะอาดสมบูรณ์ พรีเมียม และพร้อมใช้งานจริง",
    `จัดวางอย่างเป็นระเบียบบนพื้นผิวสตูดิโอสีเทาอ่อนที่เรียบมินิมอล เน้นแสดงให้เห็น${keyDetail}อย่างชัดเจน`,
    "จัดแสงไฟโชว์รูมที่สว่างเคลียร์และเป็นธรรมชาติ ไม่ดูเป็นภาพโฆษณาแฟชั่นที่รีทัชจนเนียนกริบหรือเงาเว่อร์เกินจริง"
  ].join(" ");
}

function buildSupportReferenceRoleLines({
  hasApprovedHeroAnchor = false,
  hasApprovedStudioMasterAnchor = false,
  shotKey = ""
} = {}) {
  if (hasApprovedHeroAnchor && hasApprovedStudioMasterAnchor) {
    if (isDetailSupportShotV3(shotKey)) {
      return [
        ...
        "Reference Image 3 เป็นต้นไปคือภาพสินค้าจริง ใช้ตรวจสี ทรง วัสดุ โลโก้ แพตช์ และรายละเอียดจริง"
      ];
    }
    return [
  ...
  if (!hasApprovedHeroAnchor) return [];
  return [
    "Reference Image 1 คือภาพหลักที่อนุมัติแล้ว ใช้ล็อกสินค้า โมเดล แสง โทน และความต่อเนื่องของเซ็ต",
    "Reference Image 2 เป็นต้นไปคือภาพสินค้าจริง ใช้ตรวจสี ทรง วัสดุ โลโก้ แพตช์ และรายละเอียดจริง"
  ];
}
...
function buildLeanThaiSupportPresentationLine(shotKey = "", hasApprovedHeroAnchor = false, hasApprovedStudioMasterAnchor = false) {
  ...
  const studioContinuity = hasApprovedStudioMasterAnchor
    ? " ใช้ Studio Master เป็น anchor หลักของฉาก studio และคุมให้ทั้งชุด gallery ดูต่อเนื่องกัน"
    : "";
  if (backShots.has(shotKey)) {
    return hasApprovedHeroAnchor
      ? `ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ถ้าภาพหลักมีคน ให้คงคนเดิมจากภาพหลักทั้งทรงผม สีผม ผิว สัดส่วน เสื้อด้านใน และ styling ห้ามเปลี่ยนคนเป็นคนใหม่ ยึดภาพด้านหลังต้นฉบับเป็น visual truth ห้ามเปลี่ยนเป็นหุ่นโชว์ ดัมมี่ หรือ ghost mannequin เว้นแต่ภาพต้นฉบับเป็นหุ่นจริง${studioContinuity}`
      : "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ยึดภาพด้านหลังต้นฉบับเป็น visual truth ห้ามเปลี่ยนเป็นหุ่นโชว์ ดัมมี่ หรือ ghost mannequin เว้นแต่ภาพต้นฉบับเป็นหุ่นจริง";
  }
  if (sideShots.has(shotKey)) {
    return hasApprovedHeroAnchor
      ? `ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ถ้าภาพหลักมีคน ให้คงคนเดิมจากภาพหลักทั้งโครงหน้า ทรงผม สีผม ผิว สัดส่วน เสื้อด้านใน และ styling ห้ามเปลี่ยนคนเป็นคนใหม่ ไม่ต้องยกฉาก lifestyle ของ Hero มาใหม่ แต่คุมแสง สี และความสมจริงให้ต่อเนื่องกับภาพหลัก${studioContinuity}`
      : "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า สินค้าเป็นจุดเด่นหลัก คุมแสง สี และความสมจริงให้สะอาดตา";
  }
  if (isDetailSupportShotV3(shotKey)) {
    return `ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ครอปเฉพาะรายละเอียดสินค้า ไม่ต้องมีโมเดล ไม่ต้องยกฉาก lifestyle ของ Hero มาใหม่${studioContinuity}`;
  }
  return hasApprovedHeroAnchor
    ? `ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ภาพต้องดูเป็นเซ็ตเดียวกับภาพหลักในแสง สี และความสมจริง ถ้าภาพมีคนให้คงคนเดิมจากภาพหลัก ห้ามเปลี่ยนคนเป็นคนใหม่ ไม่ต้องใส่ข้อความ ไม่ต้องแบ่งกริด ไม่ต้องแบ่งช่อง${studioContinuity}`
    : "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า สินค้าเป็นจุดเด่นหลัก ภาพสมจริง สะอาดตา ไม่ต้องใส่ข้อความ ไม่ต้องแบ่งกริด ไม่ต้องแบ่งช่อง";
}

function buildPdpSupportPresentationLine(shotKey = "", brief = {}) {
  const modelLine = isDetailSupportShotV3(shotKey)
...
function isDetailSupportShotV3(shotKey = "") {
  return [
    "lining_warmth",
    "material_or_lining_closeup",
    "texture_closeup",
...
function buildReferencePriorityLines(item = {}, { slotType = "hero" } = {}) {
  ...
  if (slotType === "support") {
    lines.push("For support images, keep the clean studio catalog direction. Use the approved hero only for product continuity, fit realism, and color calibration; do not copy its lifestyle location, snow scene, street scene, or busy background.");
  }
