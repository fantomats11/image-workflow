...
export const PROMPT_FRAMEWORK_V3_VERSION = "prompt-framework-v3.10-thai-review-hero-support";
...
function buildProductCameraFocusDirection(item = {}, shotKey = "", { slotType = "support" } = {}) {
  ...
  if (["upper_outerwear", "long_outerwear", "pants", "footwear"].includes(group)) {
    if (slotType === "hero") {
      return "Camera focus: สำหรับสินค้าชิ้นใหญ่ เช่น เสื้อ กางเกง หรือรองเท้า ใช้มุมภาพธรรมชาติที่เห็นสินค้าเต็มพอให้เข้าใจ fit, scale, silhouette และการใช้งานจริง โดยไม่จำเป็นต้องบังคับ close-up";
    }
