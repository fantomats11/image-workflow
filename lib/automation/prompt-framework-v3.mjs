...
export const PROMPT_FRAMEWORK_V3_VERSION = "prompt-framework-v3.11-thai-provider-output";
...
export function buildHeroPromptV3(item = {}) {
  ...
  return [
    ...buildThaiReviewOutputDirection(item, { slotType: "hero" }),
    "",
    ...buildThaiProductTruthLines(item, productIdentity),
    "",
    buildThaiHeroSubjectDirection(item, modelPolicy),
    "",
    ...buildThaiHumanRealismLines(modelPolicy),
    "",
    buildThaiNoAdLayoutConstraints()
  ].filter(Boolean).join("\n");
}
...
export function buildSupportPromptV3(item = {}, shotKey, index, total) {
  ...
  const productIdentity = buildProductIdentity(item);
  return [
    ...buildThaiReviewOutputDirection(item, { slotType: "support", shotKey }),
    "",
    `ภาพเสริมช็อตที่ ${index} จาก ${total}: ${describeThaiSupportShotV3(shotKey, item)}`,
    buildThaiApprovedHeroAnchorDirection(item, shotKey, modelPolicy),
    buildThaiSupportSubjectDirection(item, shotKey, modelPolicy),
    "",
    ...buildThaiProductTruthLines(item, productIdentity),
    "",
    ...buildThaiHumanRealismLines(modelPolicy),
    ...buildThaiSupportSafetyLines(item, shotKey, modelPolicy),
    "",
    "เปลี่ยนเฉพาะมุมภาพ ระยะครอป ท่าทาง หรือจุดเน้นที่จำเป็นสำหรับภาพเสริมช็อตนี้เท่านั้น อย่าเปลี่ยนสี ทรง วัสดุ สัดส่วน งานเย็บ ซิป ขอบ ขนเฟอร์ ป้ายจริง หรือรายละเอียดโครงสร้างของสินค้า",
    buildThaiNoAdLayoutConstraints()
  ].filter(Boolean).join("\n");
}
...
function buildThaiSupportSafetyLines(item = {}, shotKey = "", modelPolicy = {}) {
  const { group } = resolveProductUseCaseV3(item);
  const lines = [];
  if (["texture_closeup", "fabric_fur_zip_patch_detail", "texture_construction_closeup", "lining_warmth", "open_interior", "sole_view"].includes(shotKey)) {
