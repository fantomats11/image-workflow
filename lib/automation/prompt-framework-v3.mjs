...
export function buildSupportPromptV3(item = {}, shotKey, index, total) {
  ...
  return [
    ...
    ...buildSupportReferenceRoleLines(hasApprovedHeroAnchor),
...
function buildSupportReferenceRoleLines(hasApprovedHeroAnchor = false) {
...
function buildLeanThaiSupportPresentationLine(shotKey = "", hasApprovedHeroAnchor = false) {
  ...
  const detailShots = new Set(["lining_warmth", "material_or_lining_closeup", "texture_closeup", "texture_construction_closeup", "sole_view", "open_interior", "waistband_pocket", "hem_detail"]);
  if (backShots.has(shotKey)) {
    return hasApprovedHeroAnchor
      ? "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ถ้าภาพหลักมีคน ให้คงคนเดิมจากภาพหลักทั้งทรงผม สีผม ผิว สัดส่วน เสื้อด้านใน และ styling ห้ามเปลี่ยนคนเป็นคนใหม่ ยึดภาพด้านหลังต้นฉบับเป็น visual truth ห้ามเปลี่ยนเป็นหุ่นโชว์ ดัมมี่ หรือ ghost mannequin เว้นแต่ภาพต้นฉบับเป็นหุ่นจริง"
  ...
  if (sideShots.has(shotKey)) {
    return hasApprovedHeroAnchor
      ? "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ถ้าภาพหลักมีคน ให้คงคนเดิมจากภาพหลักทั้งโครงหน้า ทรงผม สีผม ผิว สัดส่วน เสื้อด้านใน และ styling ห้ามเปลี่ยนคนเป็นคนใหม่ ไม่ต้องยกฉาก lifestyle ของ Hero มาใหม่ แต่คุมแสง สี และความสมจริงให้ต่อเนื่องกับภาพหลัก"
      : "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า สินค้าเป็นจุดเด่นหลัก คุมแสง สี และความสมจริงให้สะอาดตา";
  }
  if (detailShots.has(shotKey)) {
    return "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ครอปเฉพาะรายละเอียดสินค้า ไม่ต้องมีโมเดล ไม่ต้องยกฉาก lifestyle ของ Hero มาใหม่";
  }
  return hasApprovedHeroAnchor
    ? "ใช้ฉาก studio ขาวหรือเทาอ่อนสะอาดแบบหน้าสินค้า ภาพต้องดูเป็นเซ็ตเดียวกับภาพหลักในแสง สี และความสมจริง ถ้าภาพมีคนให้คงคนเดิมจากภาพหลัก ห้ามเปลี่ยนคนเป็นคนใหม่ ไม่ต้องใส่ข้อความ ไม่ต้องแบ่งกริด ไม่ต้องแบ่งช่อง"
