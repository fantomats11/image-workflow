...
function toMediaAsset(candidate, index) {
  const position = candidate.kind === "hero" ? 0 : index;
  return {
    ...
    type: candidate.type,
    kind: candidate.kind,
    role: candidate.kind === "hero" ? "main_image" : "gallery_image",
    position,
    shot_key: candidate.kind === "hero" ? "hero" : candidate.kind === "studio_master" ? "studio_master" : candidate.slot,
    status: "approved",
...
function toExportSlot(candidate, index) {
  ...
  const slot = candidate.kind === "hero" ? "Hero" : candidate.kind === "studio_master" ? "Studio_Master" : candidate.slot || `Support_${index + 1}`;
  ...
  return {
    ...
    kind: candidate.kind,
    role: candidate.kind === "hero" ? "main_image" : "gallery_image",
    position: candidate.kind === "hero" ? 0 : index,
    slot: candidate.kind === "hero" ? "hero" : candidate.kind === "studio_master" ? "studio_master" : candidate.slot,
...
function buildGateBlockers({ candidateManifest, candidates }) {
  ...
  if (!candidates.some((candidate) => candidate.kind === "studio_master")) blockers.push("missing_studio_master_candidate");
...
function summarize({ candidates, exportSlots, mediaAssets, gateBlockers }) {
  return {
    ...
    studio_master_slots: exportSlots.filter((slot) => slot.kind === "studio_master").length,
...
function candidatePriority(candidate) {
  ...
  if (candidate.kind === "studio_master") return 5;
  return 10;
}

function inferKind(candidate) {
  if (/studio_master|studio master/i.test(`${candidate.type || ""} ${candidate.slot || ""} ${candidate.kind || ""}`)) return "studio_master";
