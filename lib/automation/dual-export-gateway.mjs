...
export function buildDualExportGatewayPlan({
  ...
  return {
    ...
    summary: {
      ...
      studio_master_slots: slots.filter((slot) => slot.shot_key === "studio_master").length,
      support_slots: slots.filter((slot) => slot.role === "gallery_image" && slot.shot_key !== "studio_master").length
    },
    guardrails: [
...
export function buildMediaSlots({ sku = "", assets = [] } = {}) {
  ...
  const studioMaster = normalizedAssets.filter(isStudioMasterAsset).sort(compareApprovedAssetOrder)[0] || null;
  const supports = normalizedAssets.filter((asset) => !isHeroAsset(asset) && !isStudioMasterAsset(asset)).sort(compareApprovedAssetOrder);
  return [
    hero ? toMediaSlot({ sku: normalizedSku, asset: hero, role: "main_image", position: 0, shotKey: "hero" }) : null,
    studioMaster ? toMediaSlot({ sku: normalizedSku, asset: studioMaster, role: "gallery_image", position: 1, shotKey: "studio_master" }) : null,
    ...supports.map((asset, index) => toMediaSlot({
      sku: normalizedSku,
      asset,
      role: "gallery_image",
      position: index + (studioMaster ? 2 : 1),
