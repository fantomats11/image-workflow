import { buildMediaSlots } from "./dual-export-gateway.mjs";

const MEDIA_TYPES = new Set(["hero_generated", "studio_master_generated", "support_generated", "approved_export"]);
...
export function buildMediaAssetManifest({
  ...
  const allNormalizedAssets = assets
  ...
  const normalizedAssets = allNormalizedAssets.filter((asset) => asset.status === "approved");
  const excludedUnapprovedAssets = allNormalizedAssets.filter((asset) => asset.status !== "approved");
  const assetsBySku = groupBy(normalizedAssets, (asset) => normalizeSku(asset.sku));
  const excludedBySku = groupBy(excludedUnapprovedAssets, (asset) => normalizeSku(asset.sku));
  const items = batchItems.map((item) => {
    const sku = normalizeSku(item.sku);
    const itemAssets = (assetsBySku.get(sku) || []).sort(compareManifestAssetPriority);
    const mediaSlots = buildMediaSlots({ sku, assets: itemAssets });
    return {
      sku: item.sku || "",
      ...
      excluded_unapproved_asset_count: (excludedBySku.get(sku) || []).length,
      hero_count: itemAssets.filter((asset) => asset.type === "hero_generated").length,
      studio_master_count: itemAssets.filter((asset) => asset.type === "studio_master_generated").length,
  ...
  return {
    ...
    summary: {
      ...
      studio_master_generated: normalizedAssets.filter((asset) => asset.type === "studio_master_generated").length,
      ...
      excluded_unapproved_assets: excludedUnapprovedAssets.length,
...
function inferKindFromAssetType(type) {
  ...
  if (String(type || "").includes("studio_master")) return "studio_master";
...
function manifestAssetPriority(asset) {
  ...
  if (asset.type === "studio_master_generated") score += 15;
