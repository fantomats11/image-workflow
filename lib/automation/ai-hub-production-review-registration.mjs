...
export function buildAiHubProductionReviewRegistrationPlan({
  ...
  items.forEach((item) => {
    ...
    if (!item.support_assets.length) item.blockers.push("missing_support_assets");
  });
  ...
  return {
    ...
    guardrails: [
      ...
      "include_today_hero_and_support_candidates",
...
async function persistReviewItem({ supabaseAdmin, item, plan, batch, actor, now }) {
  try {
    ...
    const supports = [];
    for (const supportAsset of item.support_assets) {
      supports.push(await persistGeneratedCandidate({ supabaseAdmin, item, asset: supportAsset, job, actor, now }));
    }
