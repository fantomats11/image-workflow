...
import { buildLineImageProxyUrl } from "./line-image-proxy-url.mjs";
...
export function buildLineKeywordBatchReviewFlex({ result = {}, batch = result.batch || {}, reviewBaseUrl = "" } = {}) {
  const batchId = String(batch.batch_id || "unknown-batch");
  const items = Array.isArray(batch.items) ? batch.items : [];
  const selection = batch.selection || {};
  const requestedText = formatRequestedCounts(selection.requested_counts);
  const selectedCount = Number(selection.selected_size || items.length || 0);
  const blockers = normalizeLineBatchBlockers(result.blockers || selection.shortfalls || []);
  const reviewUrl = buildBatchReviewUrl({ reviewBaseUrl, batchId });
  const skuRows = items.length <= 3
    ? items.map((item) => textNode(`${displayValue(item.sku, "-")} · ${displayValue(item.product_name, "สินค้า")}`, "xs", "regular", "#64748B"))
    : [textNode(`เลือกแล้ว ${selectedCount} SKU · เปิดหน้า Batch Review เพื่อดูรายการทั้งหมด`, "xs", "regular", "#64748B")];

  return {
    type: "flex",
    altText: `สร้าง Batch แล้ว: ${batchId}`,
    contents: {
      type: "bubble",
      size: "mega",
      body: {
        ...
        spacing: "md",
        contents: [
          textNode("สร้าง Batch แล้ว", "lg", "bold"),
          textNode(`${batchId} · LINE keyword`, "xs", "regular", "#64748B"),
          separatorNode(),
          textNode(`Request: ${requestedText || "-"}`, "sm", "bold"),
          textNode(`Selected: ${selectedCount} SKU`, "sm"),
          textNode(blockers.length
            ? `มีบางรายการต้องตรวจ reference ก่อนเริ่ม (${blockers.length})`
            : "พร้อมตรวจรายการก่อนเริ่ม Hero", "sm", "regular", blockers.length ? "#b45309" : "#0f766e"),
          ...skuRows,
          blockers.length ? textNode("รายละเอียดและรายการที่ต้องแก้ อยู่ใน Batch Review", "xs", "regular", "#b45309") : textNode("ขั้นต่อไป: เปิด Batch Review แล้วตรวจรายการ", "xs", "regular", "#64748B")
        ]
      },
      footer: {
        type: "box",
        ...
        contents: [
            ...
            },
          {
            ...
            action: {
              type: "postback",
              label: "ยกเลิก Batch",
              data: postbackData({ action: "cancel_batch", batch_id: batchId }),
              displayText: `Cancel ${batchId}`
            }
          }
        ]
      }
...
export function buildWordPressMediaAttachExecutionPlanFlex(executionPlan = {}) {
  const batchId = displayValue(executionPlan.batch_id, "unknown-batch");
  const summary = executionPlan.summary || {};
  const operations = Array.isArray(executionPlan.operations) ? executionPlan.operations : [];
  const rows = operations.slice(0, 8).map((operation) => {
    const status = operation.operation_status || "waiting";
    const blockers = Array.isArray(operation.blockers) && operation.blockers.length
      ? `Blockers: ${operation.blockers.join(", ")}`
      : "No blockers";
    return {
      type: "box",
      layout: "vertical",
      spacing: "xs",
      contents: [
        textNode(`${displayValue(operation.sku, "unknown-sku")} · ${displayValue(operation.role || operation.operation_type)}`, "sm", "bold"),
        textNode(status, "xs", "regular", status === "ready_for_live_write_phase" ? "#0f766e" : "#b45309"),
        textNode(displayValue(operation.idempotency_key, "missing idempotency key"), "xxs", "regular", "#64748B"),
        textNode(blockers, "xxs", "regular", "#64748B")
      ]
    };
  });
  ...
  return {
    type: "flex",
    altText: `Media attach execution plan ${batchId}: ${summary.proposed_operations || 0} operations`,
    contents: {
      type: "bubble",
      size: "mega",
      body: {
        ...
        spacing: "md",
        contents: [
          textNode("Media Attach Execution Plan", "lg", "bold"),
          textNode(`${batchId} · dry-run / no live media write`, "xs", "regular", "#64748B"),
          separatorNode(),
          textNode(`Operations: ${summary.proposed_operations || 0}`, "sm", "bold"),
          textNode(`Ready for later live phase: ${summary.ready_for_live_write_phase || 0} · Awaiting: ${summary.awaiting_final_confirmation || 0}`, "sm"),
          textNode(`Blocked: ${summary.blocked || 0} · Duplicate keys: ${summary.duplicate_idempotency_keys || 0}`, "xs", "regular", "#64748B"),
          textNode(executionPlan.requires_remote_refetch ? "Remote refetch required" : "Remote refetch not marked", "xs", "regular", "#64748B"),
          separatorNode(),
          ...rows,
          textNode("ยังไม่มีการ upload/attach/replace media หรือ publish ใน WordPress", "xs", "regular", "#b42318")
        ]
...
export function buildWordPressMediaRemoteRefetchPreflightFlex(preflight = {}) {
  const batchId = displayValue(preflight.batch_id, "unknown-batch");
  const summary = preflight.summary || {};
  const items = Array.isArray(preflight.items) ? preflight.items : [];
  const rows = items.slice(0, 8).map((item) => {
    const status = item.remote_refetch_status || item.product_remote_status || "waiting";
    const blockers = Array.isArray(item.blockers) && item.blockers.length
      ? `Blockers: ${item.blockers.join(", ")}`
      : "No blockers";
    const operationCount = Array.isArray(item.operations) ? item.operations.length : 0;
    return {
      type: "box",
      layout: "vertical",
      spacing: "xs",
      contents: [
        textNode(`${displayValue(item.sku, "unknown-sku")} · ${displayValue(item.target_site)}`, "sm", "bold"),
        textNode(`${displayValue(item.product_remote_status, "unknown")} · product ${displayValue(item.product_id, "-")}`, "xs", "regular", status === "ready_for_live_write_phase_review" ? "#0f766e" : "#b45309"),
        textNode(`operations ${operationCount} · gallery ${Array.isArray(item.current_gallery_image_ids) ? item.current_gallery_image_ids.length : 0}`, "xxs", "regular", "#64748B"),
        textNode(blockers, "xxs", "regular", "#64748B")
      ]
    };
  });
  ...
  return {
    type: "flex",
    altText: `Media remote refetch ${batchId}: ${summary.remote_products_found || 0}/${summary.item_count || 0} products`,
    contents: {
      type: "bubble",
      size: "mega",
      body: {
        ...
        spacing: "md",
        contents: [
          textNode("Media Remote Refetch Preflight", "lg", "bold"),
          textNode(`${batchId} · read-only remote state check`, "xs", "regular", "#64748B"),
          separatorNode(),
          textNode(`Products found: ${summary.remote_products_found || 0}/${summary.item_count || 0}`, "sm", "bold"),
          textNode(`Operations checked: ${summary.operation_count || 0} · Ready: ${summary.ready_items || 0} · Blocked: ${summary.blocked || 0}`, "sm"),
          textNode(`Current gallery images: ${summary.current_gallery_images || 0} · Remote media matches: ${summary.remote_media_matches || 0}`, "xs", "regular", "#64748B"),
          separatorNode(),
          ...rows,
          textNode("ยังไม่มีการ upload/attach/replace media หรือ publish ใน WordPress", "xs", "regular", "#b42318")
        ]
...
function buildBatchReviewUrl({ reviewBaseUrl = "", batchId = "" } = {}) {
  const base = safeHttpsUrl(reviewBaseUrl).replace(/\/+$/, "");
  if (!base || !batchId) return "";
  const params = new URLSearchParams({ batch_id: batchId });
  return `${base}/#batch?${params.toString()}`;
}

function formatRequestedCounts(requestedCounts = []) {
  return (requestedCounts || [])
    .map((request) => `${displayValue(request.label || request.key, "-")}=${Number(request.count || 0)}`)
    .join(" ");
}

function normalizeLineBatchBlockers(blockers = []) {
  const seen = new Set();
  const normalized = [];
  for (const blocker of blockers || []) {
    const code = String(blocker?.code || blocker || "").trim();
