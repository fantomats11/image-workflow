...
export function buildHeroReviewFlex({ batchId, item = {}, heroAsset = {}, referenceAssets = [] } = {}) {
  ...
  const heroBubble = {
    ...
    footer: {
      ...
      spacing: "sm",
      contents: [
        postbackButton("Approve hero", { action: "approve_hero", batch_id: resolvedBatchId, sku }, "primary"),
        {
          type: "box",
          layout: "horizontal",
          spacing: "sm",
          contents: [
            postbackButton("Regenerate", { action: "regenerate_hero", batch_id: resolvedBatchId, sku }, "secondary"),
            postbackButton("Needs review", { action: "needs_review", batch_id: resolvedBatchId, sku }, "secondary")
          ]
        }
      ]
    }
...
export function buildHeroReviewMessages({
  ...
  messages.push({
    type: "template",
    altText: `Hero actions: ${sku}`,
    template: {
      type: "buttons",
      title: `Hero Review ${String(sku).slice(0, 24)}`,
      text: `เลือกผลตรวจสำหรับ ${sku}`,
      actions: [
        templatePostbackAction("Approve hero", { action: "approve_hero", batch_id: resolvedBatchId, sku, generation_id: heroAsset.generation_id || "" }),
        templatePostbackAction("Regenerate", { action: "regenerate_hero", batch_id: resolvedBatchId, sku, generation_id: heroAsset.generation_id || "" }),
        ...(reviewUrl ? [templateUriAction("Open review page", reviewUrl)] : [])
      ]
    }
  });
