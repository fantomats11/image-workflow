...
function collectReferenceSources(item = {}) {
  ...
  if (referenceUrl && isDirectImageDownloadUrl(referenceUrl)) {
...
async function stageAssetLike({ source = {}, sku, role, stagingDir, fetchImpl, timeoutMs }) {
  ...
  try {
    ...
    if (!isImageContentType(contentType)) {
      throw new Error(`non_image_reference_response:${contentType || "unknown"}`);
    }
...
function isDirectImageDownloadUrl(value = "") {
  const url = String(value || "").trim();
  if (!isDirectDownloadUrl(url)) return false;
  if (/drive\.google\.com\/drive\/folders\//i.test(url) || /drive\.google\.com\/.*[?&]folders=/i.test(url)) return false;
  return true;
}
