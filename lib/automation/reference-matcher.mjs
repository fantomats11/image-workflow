...
export function matchReferenceFolderToSku({
  ...
  const pathAndFilenameText = [
    folderPath,
    ...classifiedAssets
      .filter((asset) => asset.classification?.asset_type !== "label_or_tag")
      .map((asset) => [asset.path, asset.name].filter(Boolean).join(" "))
  ].join(" ").toLowerCase();

  let matchMethod = "weak_folder_grouping";
  let confidence = 0.5;

  if (normalizedSku && pathAndFilenameText.includes(normalizedSkuLower)) {
