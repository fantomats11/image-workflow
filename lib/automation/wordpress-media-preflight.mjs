...
function buildMediaPreflightItem({ item = {}, productItem = null, mediaAssets = [] } = {}) {
  ...
  return {
    ...
    proposed_gallery_images: supportAssets.map((asset, index) => compactMediaAsset({ ...asset, role: "gallery_image", position: index + 1 })),
    proposed_images: [
      heroAsset ? compactMediaAsset({ ...heroAsset, role: "main_image", position: 0 }) : null,
      ...supportAssets.map((asset, index) => compactMediaAsset({ ...asset, role: "gallery_image", position: index + 1 }))
    ].filter(Boolean),
