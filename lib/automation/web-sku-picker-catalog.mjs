...
export function normalizeWebSkuPickerRow(row = {}, index = 0) {
  ...
  const referenceParentFolderId = cleanText(firstValue(row, ["reference_parent_folder_id", "parent_folder_id"]));
  ...
  const item = {
    ...
    reference_parent_folder_id: referenceParentFolderId,
    reference_lookup_key: cleanText(firstValue(row, ["reference_lookup_key"])) || sku,
...
export function buildWebSkuReferenceCards({
  ...
  for (const asset of driveAssets.slice(0, 8)) {
    ...
    const generationUrl = cleanText(asset.generation_url || asset.staged_url || asset.staged_public_url || asset.public_url);
    const explicitBlockerCode = cleanText(asset.blocker_code || asset.staging_error_code);
    const explicitBlockerMessage = cleanText(asset.blocker_message || asset.staging_error_message_th);
    ...
    const blockers = isUsableReference
      ? generationUrl
        ? []
        : [{
          code: explicitBlockerCode || "reference_not_staged",
          message_th: explicitBlockerMessage || "ยัง stage รูปจาก Drive เข้า Supabase Storage ไม่สำเร็จ"
        }]
      : [{
    ...
    cards.push({
      ...
      file_name: fileName,
      drive_mime_type: cleanText(asset.drive_mime_type || asset.mimeType || asset.mime_type),
      mime_type: cleanText(asset.mimeType || asset.mime_type || asset.drive_mime_type),
      width: Number(asset.width || 0),
      height: Number(asset.height || 0),
      preview_url: previewUrl,
      generation_url: generationUrl,
      staged_url: cleanText(asset.staged_url || asset.generation_url || asset.staged_public_url || asset.public_url),
      storage_bucket: cleanText(asset.storage_bucket),
      storage_key: cleanText(asset.storage_key || asset.storage_path),
      storage_path: cleanText(asset.storage_path || asset.storage_key),
      staging_status: cleanText(asset.staging_status || (generationUrl ? "staged_to_supabase" : "")),
      upload_reused: asset.upload_reused === true,
      blocker_code: generationUrl ? "" : explicitBlockerCode,
      blocker_message: generationUrl ? "" : explicitBlockerMessage,
