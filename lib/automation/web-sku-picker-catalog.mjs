...
export async function loadWebSkuPickerCatalogSnapshot({
  catalogPath = process.env.WEB_SKU_PICKER_CATALOG_CSV || process.env.LINE_KEYWORD_GENERATION_CATALOG_CSV || WEB_SKU_PICKER_DEFAULT_CATALOG_PATH
} = {}) {
  const resolvedPath = path.resolve(catalogPath || WEB_SKU_PICKER_DEFAULT_CATALOG_PATH);
...
export function searchWebSkuPickerCatalog({
  ...
  const normalizedRows = normalizeWebSkuPickerRows(rows);
...
export function findWebSkuPickerItemBySku(rows = [], sku = "") {
  ...
  return normalizeWebSkuPickerRows(rows).find((item) => item.sku === normalizedSku) || null;
}
...
export function buildWebSkuReferenceCards({
  ...
  for (const asset of driveAssets.slice(0, 8)) {
    ...
    cards.push({
      ...
      stage_available: Boolean(previewUrl && isUsableReference),
      ...
      generation_url: previewUrl,
      warnings: buildReferenceWarnings({ item, asset, isUsableReference }),
      blockers: isUsableReference ? [] : [{
...
function buildReferenceReadiness(item, references = []) {
  ...
  const status = !hasReference
    ? "blocked"
    : stageableCount || (isReferenceVerified(item.reference_verified) && hasReference)
      ? "ready"
      : "warning";
  return {
    status,
    label_th: status === "ready"
      ? "มี reference จาก catalog/Drive พร้อมตรวจ"
      : status === "warning"
        ? "มี reference แต่ต้องตรวจเพิ่ม"
        : "ยังไม่มี reference พร้อมใช้",
    reference_count: Math.max(references.length, hasReference ? 1 : 0),
    usable_reference_count: stageableCount || (hasReference ? 1 : 0),
...
function buildReferenceWarnings({ item = {}, asset = {}, isUsableReference = true } = {}) {
  const warnings = [];
