...
export async function registerAutomationBatch(batch, { source = "script", lineUserId = "" } = {}) {
  ...
  const existingMetadata = isPlainObject(existingBatch?.metadata) ? existingBatch.metadata : {};
    ...
    .upsert(
      {
        batch_key: batchKey,
        source,
        status: existingBatch?.status || "awaiting_approval",
        dry_run: dryRun,
        ...
        metadata: {
          ...existingMetadata,
          ...buildAutomationBatchMetadata(batch)
        }
      },
  ...
  const { data: existingItems, error: existingItemsError } = await supabaseAdmin
    ...
    .select("sku, status, metadata")
    .eq("batch_id", automationBatch.id);
  if (existingItemsError) throw existingItemsError;
  for (const item of existingItems || []) {
    existingItemsBySku.set(String(item.sku || "").trim(), item);
  }

  const items = (batch.items || [])
    .map((item) => {
      const payload = buildAutomationBatchItemPayload(automationBatch.id, item);
      const existingItem = existingItemsBySku.get(payload.sku);
      if (existingItem?.status) payload.status = existingItem.status;
      if (isPlainObject(existingItem?.metadata)) {
        payload.metadata = {
