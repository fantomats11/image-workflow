...
function buildPlanItem({ item = {}, index = 0, assets = [], referenceResolution = null, modelInputStaging = null, prioritySupportCount }) {
  const normalized = normalizeBatchItem(item);
  const promptItem = { ...normalized, item_index: index };
  const supportShots = splitSupportShots(normalized.support_shots);
  const heroAsset = chooseHeroAsset(assets);
  const approvedHeroAnchor = chooseApprovedHeroAnchor(assets) ||
    (isHeroApprovedByItem(item) ? chooseGeneratedHeroAnchor(assets) : null);
...
function buildRequest({
  ...
  return {
    request_id: `${item.sku || `item-${index + 1}`}:${slot}`,
    sku: item.sku,
    kind,
    slot,
    sequence,
