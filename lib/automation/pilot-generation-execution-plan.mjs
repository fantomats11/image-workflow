...
export function buildPilotGenerationExecutionPlan({
  ...
  return {
    ...
    summary: {
      ...
      studio_master_requests: requests.filter((request) => request.kind === "studio_master").length,
      ...
      pending_studio_master_approval_for_support: requests.filter((request) => request.blockers?.includes("support_requires_approved_studio_master_anchor")).length,
...
function buildPlanItem({
  ...
  const metadataApprovedHeroAnchor = normalized.approved_hero_anchor
    ...
    : null;
  const metadataApprovedStudioMasterAnchor = normalized.approved_studio_master_anchor
    ? normalizeMediaAsset(normalized.approved_studio_master_anchor)
    : null;
  const heroAsset = chooseHeroAsset(assets) ||
    (isHeroApprovedByItem(item) ? metadataApprovedHeroAnchor : null);
  const studioMasterAsset = chooseStudioMasterAsset(assets) ||
    (isStudioMasterApprovedByItem(item) ? metadataApprovedStudioMasterAnchor : null);
  ...
  const approvedHeroAnchor = chooseApprovedHeroAnchor(assets) ||
    ...
    (isHeroApprovedByItem(item)
      ...
      : null);
  const approvedStudioMasterAnchor = chooseApprovedStudioMasterAnchor(assets) ||
    chooseApprovedStudioMasterAnchor(metadataApprovedStudioMasterAnchor ? [metadataApprovedStudioMasterAnchor] : []) ||
    (isStudioMasterApprovedByItem(item)
      ? chooseGeneratedStudioMasterAnchor(assets) || metadataApprovedStudioMasterAnchor
      : null);
  ...
  if (approvedHeroAnchor && supportShots.length > 0) {
    const shouldRegenerateStudioMaster = actionContext.forceStudioMasterRegeneration && targetedByReviewAction;
    if (studioMasterAsset && !shouldRegenerateStudioMaster) {
      skippedExistingSlots.push({ slot: "studio_master", reason: "existing_studio_master_asset", asset_id: studioMasterAsset.id });
    } else {
      const studioMasterBlockers = [];
      if (!approvedHeroAnchor.local_path) studioMasterBlockers.push("approved_hero_anchor_requires_local_file");
      const prompt = buildStudioMasterPromptV3(promptItem);
      requests.push(buildRequest({
        item: promptItem,
        index,
        slot: "studio_master",
        ...
        requestBlockers: studioMasterBlockers,
        priorityRequired: true,
        sequence: 2
      }));
    }
  ...
  supportShots.forEach((shotKey, shotIndex) => {
    ...
    if (existingAsset && isReusableSupportAsset(existingAsset, approvedHeroAnchor, approvedStudioMasterAnchor)) {
    ...
    if (!approvedStudioMasterAnchor) supportBlockers.push("support_requires_approved_studio_master_anchor");
    else if (!approvedStudioMasterAnchor.local_path) supportBlockers.push("approved_studio_master_anchor_requires_local_file");
    const supportPromptItem = {
      ...promptItem,
      ...(approvedHeroAnchor ? { approved_hero_anchor: compactHeroAnchor(approvedHeroAnchor) } : {}),
      ...(approvedStudioMasterAnchor ? { approved_studio_master_anchor: compactStudioMasterAnchor(approvedStudioMasterAnchor) } : {})
    };
  ...
  return {
    ...
    approved_studio_master_anchor: approvedStudioMasterAnchor ? compactStudioMasterAnchor(approvedStudioMasterAnchor) : null,
    support_requires_hero_approval: supportShots.length > 0 && !approvedHeroAnchor,
    support_requires_studio_master_approval: supportShots.length > 0 && !approvedStudioMasterAnchor,
...
function buildRequest({
  ...
  const heroInput = heroAnchorAsset ? compactHeroAnchorAsModelInput(heroAnchorAsset) : null;
  const studioMasterInput = studioMasterAnchorAsset ? compactStudioMasterAnchorAsModelInput(studioMasterAnchorAsset) : null;
  const modelInputFiles = buildModelInputFilesForRequest({
    kind,
    slot,
    heroInput,
    studioMasterInput,
  ...
  return {
    ...
    approved_hero_anchor: ["support", "studio_master"].includes(kind) && heroAnchorAsset ? compactHeroAnchor(heroAnchorAsset) : null,
    approved_studio_master_anchor: kind === "support" && studioMasterAnchorAsset ? compactStudioMasterAnchor(studioMasterAnchorAsset) : null,
...
function normalizeBatchItem(item = {}) {
  ...
  return {
    ...
    approved_studio_master_anchor: isPlainObject(merged.approved_studio_master_anchor) ? merged.approved_studio_master_anchor : null
  };
}
...
function isStudioMasterApprovedByItem(item = {}) {
  const action = String(
    item.metadata?.web_review_action?.last_action ||
    item.metadata?.web_review_action?.action ||
    item.metadata?.line_action?.last_action ||
  ...
  return action === "approve_studio_master";
}
...
function buildGenerationActionContext(task = {}) {
  ...
  const reviewScoped = ["approve_hero", "regenerate_hero", "approve_studio_master", "regenerate_studio_master"].includes(action);
  return {
    action,
    targetSku,
    targetGenerationId,
    reviewScoped,
    forceSupportAfterHeroApproval: action === "approve_hero" || action === "approve_studio_master",
    forceHeroRegeneration: action === "regenerate_hero",
    forceStudioMasterRegeneration: action === "regenerate_studio_master"
  };
}
...
function buildSkippedReviewActionItem({
  ...
  return {
    ...
    support_requires_studio_master_approval: false,
...
function chooseStudioMasterAsset(assets) {
  return assets.find((asset) => isStudioMasterAsset(asset) && isApprovedStudioMasterAsset(asset)) ||
    assets.find((asset) => isStudioMasterAsset(asset)) ||
    null;
}

function chooseApprovedStudioMasterAnchor(assets) {
  return assets.find((asset) => isStudioMasterAsset(asset) && isApprovedStudioMasterAsset(asset)) || null;
}

function chooseGeneratedStudioMasterAnchor(assets) {
  return assets.find((asset) => isStudioMasterAsset(asset)) || null;
}
...
function isStudioMasterAsset(asset = {}) {
  return asset.kind === "studio_master" ||
    asset.shot_key === "studio_master" ||
    asset.slot === "studio_master" ||
    asset.type === "studio_master_generated";
}

function isApprovedStudioMasterAsset(asset = {}) {
  return asset.status === "approved" || Boolean(asset.approval_id || asset.approved_at);
}

function isReusableSupportAsset(asset = {}, approvedHeroAnchor = null, approvedStudioMasterAnchor = null) {
  if (asset.type === "approved_export" || asset.status === "approved" || asset.approval_id) return true;
  if (!approvedHeroAnchor || !approvedStudioMasterAnchor) return false;
  return Boolean(
    asset.approved_studio_master_anchor_id ||
    asset.studio_master_anchor_asset_id ||
    asset.approved_hero_anchor_id ||
    asset.approved_hero_anchor_asset_id ||
    asset.hero_anchor_asset_id
  );
}
...
function normalizeMediaAsset(asset = {}) {
  return {
    ...
    approved_studio_master_anchor_id: asset.approved_studio_master_anchor_id || asset.studio_master_anchor_asset_id || null,
    status: asset.status || ""
  };
}
...
function compactAsset(asset) {
  return {
    ...
    approved_studio_master_anchor_id: asset.approved_studio_master_anchor_id
  };
}
...
function compactStudioMasterAnchor(asset = {}) {
  return {
    id: asset.id || null,
    sku: asset.sku || "",
    type: asset.type || "",
    kind: asset.kind || "studio_master",
    shot_key: asset.shot_key || "studio_master",
    status: asset.status || "",
    url: asset.url || asset.public_url || "",
    public_url: asset.public_url || asset.url || "",
    local_path: asset.local_path || "",
...
function compactStudioMasterAnchorAsModelInput(asset = {}) {
  return {
    source_name: "approved_studio_master_anchor",
    source_role: "approved_studio_master_anchor",
    local_path: asset.local_path || "",
    file_name: asset.file_name || "approved-studio-master.png",
    file_size: asset.file_size || 0,
    sha256: asset.sha256 || "",
    staging_status: asset.local_path ? "staged_local_file" : "missing_local_file",
    source_url: asset.url || asset.public_url || "",
...
function buildModelInputFilesForRequest({
  kind,
  slot,
  heroInput = null,
  studioMasterInput = null,
  stagedReferenceInputs = []
} = {}) {
  if (kind === "studio_master") {
    return [heroInput, ...stagedReferenceInputs].filter(Boolean);
  }
  if (kind !== "support") return stagedReferenceInputs;
  if (isDetailSupportShot(slot)) {
    return [studioMasterInput, heroInput, ...stagedReferenceInputs].filter(Boolean);
  }
  return [heroInput, studioMasterInput, ...stagedReferenceInputs].filter(Boolean);
}

function isDetailSupportShot(shotKey = "") {
  return [
    "lining_warmth",
