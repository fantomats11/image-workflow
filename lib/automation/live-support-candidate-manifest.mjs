...
export function buildLiveSupportCandidateManifest({
  ...
  const normalizedSku = sku || job.sku || heroAsset.sku || "";
  const metadata = buildSkuMetadata({ sku: normalizedSku, job });
  const manifestBlockers = buildManifestBlockers({ heroAsset, supportAssets, decisionState });
...
function buildManifestBlockers({ heroAsset, supportAssets, decisionState }) {
  const blockers = [];
