...
function buildMediaMappingPreflightRow({ task, preflight }) {
  ...
  return {
    ...
    summary: {
      ...
      exportReady: Number(summary.export_ready || 0),
      driveExported: Number(summary.drive_exported || 0),
      wooPreflightReady: Number(summary.woo_preflight_ready || summary.ready_for_media_proposal || 0),
      blockedByConflict: Number(summary.blocked_by_conflict || 0),
      awaitingFinalConfirmation: Number(summary.awaiting_final_confirmation || 0)
    },
    items: items.slice(0, 20).map((item) => ({
      sku: safeText(item.sku || ""),
