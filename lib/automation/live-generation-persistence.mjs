  ...
  for (const result of results) {
    ...
    generatedAssets.forEach((asset, index) => {
      ...
      items.push({
        ...
        type: asset.type || assetTypeForKind(asset.kind || result.kind || context.kind || ""),
...
async function persistGeneratedAssetItem({ supabaseAdmin, item, actorId, now }) {
  try {
    ...
    await updateJobStatus({ supabaseAdmin, jobId: job.id, status: jobStatusForKind(item.kind) });
...
function jobStatusForKind(kind = "") {
  if (kind === "studio_master") return "studio_master_ready";
  if (kind === "support") return "support_ready";
  return "hero_ready";
}
