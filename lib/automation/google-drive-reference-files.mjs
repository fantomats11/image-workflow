...
export async function listGoogleDriveReferenceImageFiles(drive, folderId, {
  ...
  if (isGoogleDriveAuthError(driveListError)) {
    throw driveListError;
  }
...
export async function findGoogleDriveChildFolderByExactName(drive, parentFolderId, folderName, {
  requestTimeoutMs = 8_000,
  pageSize = 100
} = {}) {
  const rootFolderId = cleanDriveId(parentFolderId);
  const targetName = normalizeFolderMatchName(folderName);
  if (!drive || !rootFolderId || !targetName) return null;
  const children = await listDriveFolderChildren(drive, rootFolderId, { pageSize, requestTimeoutMs });
  const matches = children
    .filter((child) => child.mimeType === GOOGLE_DRIVE_FOLDER_MIME_TYPE)
    .filter((child) => normalizeFolderMatchName(child.name) === targetName);
  return matches.length === 1 ? normalizeDriveReferenceFile(matches[0], { parentFolderId: rootFolderId }) : null;
}
...
export function isGoogleDriveAuthError(error) {
  ...
  const message = String(error.message || error.response?.data?.error_description || error.response?.data?.error?.message || "").toLowerCase();
  return status === 401
    || code === "invalid_grant"
    || code === "invalid_token"
...
function normalizeFolderMatchName(value = "") {
  return String(value || "")
    .normalize("NFKC")
    .trim()
    .toLowerCase()
