...
export function buildLineKeywordBatchFromCatalog({
  ...
  return {
    ok: parsedCommand.recognized === true && blockers.length === 0 && selectedItems.length > 0,
    batch,
    blockers
  };
}

export async function loadLineKeywordBatchCatalogSnapshot({ outputsDir } = {}) {
...
function buildSuccessReplyText(batch) {
  ...
  return [
    "สร้าง batch จาก LINE keyword แล้ว",
    `Batch: ${batch.batch_id}`,
    `Request: ${requestedText || "-"} · Selected: ${batch.items?.length || 0}`,
    "ขั้นต่อไป: กด Approve batch ในการ์ดนี้ เพื่อเริ่ม flow hero ก่อน support"
  ].join("\n");
}
