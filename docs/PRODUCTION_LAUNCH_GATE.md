...
**Recommendation: Go for controlled pilot, not wider rollout yet.**

เหตุผล:

- E2E contract, worker mode, security boundary, LINE batch path และ smoke scripts มี guardrail ครบพอสำหรับ pilot ขนาดเล็ก
- WordPress/WooCommerce live write ยังถูกปิดโดยตั้งใจ
- ยังมี accepted risks ด้าน UX, manual staff training, Google Drive dependency และ provider/cost monitoring จึงยังไม่ควรเปิดกว้างโดยไม่มี admin เฝ้าดู

Manual production smoke record is required before pilot. ห้ามเปลี่ยน gate ที่ยังเป็น `Must verify manually` เป็น pass จนกว่าจะมีหลักฐานจาก production smoke จริงอย่างน้อย 1 batch
...
| LINE webhook verified | Invalid signature ถูก reject, valid flow ยังทำงาน | Implemented/tested by automation | ต้อง monitor live endpoint หลัง deploy |
| LINE batch creates Batch Review link | `BATCH รองเท้า=1 เสื้อ=1` ส่ง link `#batch?batch_id=...` | Implemented/tested by automation | Manual production smoke ยังต้องทำ |
| Batch Review confirm works | Confirm idempotent และ enqueue Hero ตาม existing rules | Implemented/tested by automation | ห้าม confirm ซ้ำแล้วเกิด duplicate generation |
...
| Reference diagnostics ชัด | Staff เห็นว่า reference ไหนใช้จริง/ถูกตัดออก | Pending |
...
ก่อนเริ่มให้เปิด `docs/PRODUCTION_SMOKE_RECORD_TEMPLATE.md` และบันทึกผลทุกข้อที่สำคัญ
