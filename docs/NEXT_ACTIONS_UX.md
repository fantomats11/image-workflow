...
| Export มีปัญหา | `exportStatus` มี `failed/error/blocked` หรือ `canRetryExport=true` | เปิดงานที่ต้องทำ |
| งาน failed ที่ควรแจ้ง admin | `workflowState` เป็น `hero_failed`, `support_failed`, `failed`, `partially_failed` หรือ status ฝั่ง generation เป็น failed | เปิดงานที่ต้องทำ |
...
- ถ้าไม่มีงาน ระบบแสดง empty state: `วันนี้ยังไม่มีงานที่ต้องทำต่อ`

## Admin behavior

- Admin เห็นหน้า `#next` เป็น default เหมือน staff
- Admin ยังเข้า `#settings`, `#monitoring`, `#costs` ได้จาก sidebar
- Admin เห็น `Admin summary` แบบสั้นเมื่อมี export problem, failed, หรือ recovery action แต่ summary นี้ไม่แทนที่ Monitoring
...
7. ถ้า `/api/jobs` ส่งงาน failed/export failed ควรเห็น card ที่เกี่ยวข้อง และปุ่มเดียวคือ `เปิดงานที่ต้องทำ`
